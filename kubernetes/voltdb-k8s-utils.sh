...
make_statefulset() {
    ...
    cont_image=$IMAGE_TAG
    [[ -n $REP ]] && cont_image=$REP/$IMAGE_TAG
    SED="sed -i"
    [[ $OSTYPE =~ darwin ]] && SED="sed -i.bak"
    $SED "s:--clusterName--:$CLUSTER_NAME:g"          $MANIFEST
    $SED "s+--containerImage---+$cont_image+g"        $MANIFEST
...
purge_persistent_claims() {
    ...
    # !!! You will lose your related persistent volumes (all your data), with no possibility of recovery
...
usage() {
    ...
    echo "   config-file:"
    echo "      Your customized copy of config_template.cfg"
    echo "   options:"
    ...
    echo "      -C --configure-voltdb           Generates a deployment manifest (yaml file) for your config"
    echo "      -S --start-voltdb               Run kubectl create <manifest> or kubectl .. --replicas="
    echo "      -P --stop-voltdb                Run voltadmin pause --wait; kubectl scale <manifest> replicas=0"
    echo "      -R --restart-voltdb             Shorthand for -P -S"
